...
local function ScrollAreasTab_Create()
	...
	button:SetClickHandler(
		function (this)
			...
			if (MSBTPopups and MSBTPopups.ShowScrollAreaConfig) then
				MSBTPopups.ShowScrollAreaConfig()
			else
				-- Fallback: Show a message and reopen the main frame
				print("MSBT Options: Scroll area configuration is not available in this version")
				MSBTOptMain.ShowMainFrame()
			end
		end
	)
