...
local function ShowScrollAreaConfig()
	...
	if not MSBTAnimations or not MSBTAnimations.scrollAreas then
		print("MSBT: Scroll areas not available - missing dependencies")
		return
	end

	-- Create the frame if it hasn't already been.
	if (not popupFrames.scrollAreaConfigFrame) then 
		popupFrames.scrollAreaConfigFrame = CreateScrollAreaConfig() 
	end
...
-- CLASS_NAMES = LocalizedClassList(); -- Function not defined, commenting out to prevent error
CLASS_NAMES = "DEATHKNIGHT DRUID HUNTER MAGE MONK PALADIN PRIEST ROGUE SHAMAN WARLOCK WARRIOR DEMONHUNTER EVOKER"
